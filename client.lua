...
AddEventHandler('iens:repair', function()
	if isPedDrivingAVehicle() then
		...
		if IsNearMechanic() then
			...
			notification("~g~The mechanic fixed your shit bruh! Now get outta hear before he steals your shoes.")
			return
		end
		if GetVehicleEngineHealth(vehicle) < cfg.cascadingFailureThreshold + 5 then
			if GetVehicleOilLevel(vehicle) > 0 then
				...
				healthPetrolTankLast=750.0
					SetVehicleEngineOn(vehicle, true, false )
				local currentOilLevel = GetVehicleOilLevel(vehicle)
				local newOilLevel = math.max(0.0, (currentOilLevel / 3.0) - 0.5)
				...
				notification("~r~Your vehicle was too emotionally damaged. Unable to repair!")
			end
		else
	...
	while true do
		...
		if isPedDrivingAVehicle() then
			...
			if pedInSameVehicleLast == true then
				...
				-- Check if the vehicle still exists and is valid before trying to restore handling
